...
class Llama3:
    ...
    def _load_model(self):
        if LLM is None:
            raise ImportError("vllm is required for Llama3Counselor/CactusLlama3Counselor. Install it or use a GPT/Gemini counselor model.")
        self.llm = LLM(
...
class Counselor(Llama3):
    def __init__(
        self, model_name_or_path, prompt_name="agent_cactus_llama3.txt", device="cuda"
    ):
...
class CactusLlama3Counselor(Counselor):
    def __init__(
        self,
        model_name_or_path,
...
class Llama3Counselor(Counselor):
    def __init__(
        self, model_name_or_path, prompt_name="agent_llama3.txt", device="cuda"
    ):
        super().__init__(
            model_name_or_path=model_name_or_path,
            prompt_name=prompt_name,
    ...
    def respond(self, client_statement, max_tokens=256):
        self._add_to_history("Client", client_statement)
