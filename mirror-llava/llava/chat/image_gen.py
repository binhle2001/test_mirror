...
class ImageGenerator:
    def __init__(self, 
        ...
        self.verification_url = IMG_VERIFICATION_URL.rstrip("/")
    ...
    def verify_image(self, image_path, base_image_path, client_statement):
        if not self.verification_url:
            raise ValueError("IMG_VERIFICATION_URL is not set")
        ...
        with open(image_path, 'rb') as img_file:
            with open(base_image_path, 'rb') as base_file:
                ...
                response = requests.post(f"{self.verification_url}/verify", files=files, data=data, timeout=60)
    ...
    def run(self, origin, gender, dialogue, statement, dialog_idx, turn_idx):
        ...
        for _ in range(try_num):
            ...
            client_statements = extract_text_in_parentheses(statement)
            client_statement = client_statements[0] if client_statements else statement

            is_ok = self.verify_image(
...
class PromptGenerator:
    def __init__(self):
        self.description_url = IMG_DESCRIPTION_URL.rstrip("/")

    def get_prompt(self, gender, dialogue, utter, **kwargs):
        if not self.description_url:
            raise ValueError("IMG_DESCRIPTION_URL is not set")

        data = {'history': dialogue, 'client_utt': utter}
        response = requests.post(f"{self.description_url}/describe", data=data, timeout=60)
        response.raise_for_status()

        payload = response.json()
        gen_prompts = self.parse_description(payload.get("response", ""))
